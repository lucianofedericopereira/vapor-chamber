...
> library has since aligned through rc.3, rc.4, rc.5 and rc.6 (v1.14.0 through
> v1.17.0) without a full bench re-run recorded here, so read every absolute
> below as a beta-era snapshot. The per-release alignment findings for the RC
> window live in CHANGELOG.md; the guards that actually catch a regression -
