...
Vapor Chamber is a <!-- vc:sizeCore -->4.1<!-- /vc:sizeCore --> KB-brotli command bus built for Vue Vapor. It provides a semantic,
...
receive **3.3x more LLM output per input token** than violating functions (0.141 vs 0.043
...
Both official libraries exist, are good, and are the right default for a generic
Vue SPA. This repo ships its own router and its own store anyway, so the
...
`useCommandState` dispatch path (interleaved same-process A/B): array-state dispatch is ~3.4x
faster with `shallowRef`, scalar signals ~1.2x faster, with lower per-write allocation. Direct
...
This pattern is unchanged and still correct for an app already on Pinia.
...
shipped" for twelve minor versions, and §16 has recorded that as a known error
...
| full      | SPAs that grew big (realtime + undo/redo + persistence)   | <!-- vc:sizeIifeFull -->11.2<!-- /vc:sizeIifeFull --> KB |
...
- the store keeps state as one shallow ref swapped wholesale, measured at about
  3.4x deep refs on array-state dispatch;
- the Vapor outlet turns a navigation into a sentinel-keyed `DynamicFragment`
  update, so the same record resolves to the same component and the update
  no-ops - reuse rather than remount on param-only navigations.
...
replacement gets atomic single-trigger updates with no deep watcher existing at
all, and the render side has no diff to skip. The composed stack has zero deep
...
| Bundle size | ~50KB | ~15KB | ~14KB | <!-- vc:sizeCore -->4.1<!-- /vc:sizeCore --> KB brotli core |
...
- **<!-- vc:sizeCore -->4.1<!-- /vc:sizeCore --> KB brotli dispatch core** - measured ([BUNDLE-SIZES.md](./BUNDLE-SIZES.md)); this line read "~4 KB gzipped" while every other size in this document is brotli, which is a different number for the same artifact
...
tests/                           (<!-- vc:testFiles -->138<!-- /vc:testFiles --> files, <!-- vc:tests -->2075<!-- /vc:tests --> tests)
```
