...
- **Vue 3.6.0-beta.16 aligned** — signals, `onScopeDispose`, `getCurrentScope`, alien-signals internals; tracked per beta (see CHANGELOG)
...
Requires Node ≥20.19. Vue is an **optional** peer dep (≥3.5 for composables, ≥3.6.0-beta.16 for the full Vapor surface) — the core bus runs without Vue entirely.
...
|-----------|----------------------------------------------|---------------------------------------|-------|--------|--------|
| core      | Sprinkled JS on server-rendered pages (Blade, Rails, Django, .NET MVC, WordPress). You dispatch user actions to a backend over HTTP. | `vapor-chamber-core.iife.js`     | 24.4 KB | 7.0 KB  | 7.8 KB  |
| elements  | Embeddable widgets (chat bubbles, checkout buttons, third-party drop-ins). You ship a `<vc-widget>` custom element. | `vapor-chamber-elements.iife.js` | 25.9 KB | 7.4 KB  | 8.3 KB  |
...
| v1.7.0 (unreleased, beta.16) | **10.2** | **7.0** | **7.4** | Vite 8 / rolldown minifier — **smaller, no regression** |
...
**Requirements:** Node.js ≥20.19.0 | Vue ≥3.5.0 (composables) or ≥3.6.0-beta.16 (full Vapor) — optional peer dep | Vite ≥7.0.0 + `@vitejs/plugin-vue` ≥5.0.0 (only required for the `vapor-chamber/vite` HMR plugin and Vapor SFC support)
